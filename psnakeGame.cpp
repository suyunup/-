...
void SnakeGame::make_snake(){
    ...
    if(map[height_win/2][width_win/2 - 2] == 1||map[height_win/2][width_win/2 - 1] == 1||map[height_win/2][width_win/2] == 1 || map[height_win/2][width_win/2 + 1] == 1// 벽생성된 곳에 뱀이 생성되지 않도록
      ||map[height_win/2][width_win/2 + 2] == 1 || map[height_win/2][width_win/2 + 3] == 1){
        ...
        snake.push_back(Point(height_win/2,width_win/2));
        snake.push_back(Point(height_win/2, width_win/2+1));
        snake.push_back(Point(height_win/2, width_win/2+2));
        snake.push_back(Point(height_win/2, width_win/2+3));
...
void SnakeGame::new_gate(){//게이트 생성(wall의 9가 게이트)
  init_pair(1,COLOR_WHITE,COLOR_BLUE);//색 attribute 설정
  init_pair(2,COLOR_BLUE,COLOR_WHITE);//스코어
  init_pair(3,COLOR_WHITE,COLOR_RED);//미션
  init_pair(4,COLOR_GREEN,COLOR_WHITE);//뱀
...
void SnakeGame::mission_show(WINDOW *w){
  ...
  switch (stage) {
    ...
    mvwprintw(w, 4, 4, "1");//테스트용 게이트
    if(num_gate >= 1){
      mvwprintw(w, 4, 6, "(V)");
      gate_mission = true;
    }else{mvwprintw(w, 4, 6, "( )");}
    // mvwprintw(w, 4, 4, "11");//////////////원래 게이트
    // if(num_gate >= 11){
    //   mvwprintw(w, 4, 6, "(V)");
    //   gate_mission = true;
    // }else{mvwprintw(w, 4, 6, "( )");}
  ...
  //
  //
  // Point SnakeGame::search_output(int row, int col, Point output_gate,int *pre){
...
void SnakeGame::meet_gate(int row, int col, int dir, int *pre){
  ...
  goHead(dir, search_output(row, col, output_gate,pre).row,search_output(row, col, output_gate,pre).col,1);
}// end meet_gate
